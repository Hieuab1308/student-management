        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <h1 class="mb-4">Chỉnh sửa điểm</h1>
                    <form action="{{ route('scores.update', $score->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="student_id" class="form-control mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                                @foreach ($students as $student)
                                    <option value="{{ $student->id }}" {{ $student->id == $score->student_id ? 'selected' : '' }}>
                                        {{ $student->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="subject" class="form-control mt-1 block w-full rounded-md border-gray-300 shadow-sm" value="{{ $score->subject }}" required>
                        </div>
                        <div class="mb-4">
                            <label for="score" class="block text-sm font-medium text-gray-700">Điểm</label>
                            <input type="number" name="score" class="form-control mt-1 block w-full rounded-md border-gray-300 shadow-sm" value="{{ $score->score }}" step="0.1" required>
                        </div>
